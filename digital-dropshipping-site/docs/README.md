# Documentation Index

Welcome to the Uniti documentation! All guides are organized here for easy access.

## 📚 Getting Started

- **[SETUP.md](SETUP.md)** - Complete development environment setup
- **[QUICK_DEPLOY.md](QUICK_DEPLOY.md)** - Deploy to production in 10 minutes

## 🚀 Deployment & Operations

- **[DEPLOYMENT.md](DEPLOYMENT.md)** - Complete deployment guide
- **[AUTOMATION_SETUP.md](AUTOMATION_SETUP.md)** - CI/CD automation overview

## 📖 Organization Guides

- **[FILE_ORGANIZATION.md](FILE_ORGANIZATION.md)** - File organization guide
- **[ORGANIZATION_SUMMARY.md](ORGANIZATION_SUMMARY.md)** - Organization summary

## 📖 Main Documentation

- **[README.md](../README.md)** - Project overview and main documentation
- **[PROJECT_STRUCTURE.md](../PROJECT_STRUCTURE.md)** - Complete project structure

## 📁 Documentation Structure

```
docs/
├── README.md (this file)
├── SETUP.md
├── QUICK_DEPLOY.md
├── DEPLOYMENT.md
├── AUTOMATION_SETUP.md
├── FILE_ORGANIZATION.md
└── ORGANIZATION_SUMMARY.md
```

## 🔍 Quick Links

- **New to the project?** → Start with [SETUP.md](SETUP.md)
- **Want to deploy?** → Follow [QUICK_DEPLOY.md](QUICK_DEPLOY.md)
- **Need CI/CD?** → Read [AUTOMATION_SETUP.md](AUTOMATION_SETUP.md)
- **Full deployment?** → See [DEPLOYMENT.md](DEPLOYMENT.md)
- **Understanding file structure?** → See [FILE_ORGANIZATION.md](FILE_ORGANIZATION.md)

